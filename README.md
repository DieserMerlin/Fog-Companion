...
Default callout graphics in this repository belong to [Hens](https://hens333.com). I got his approval to use them for the Fog Companion!
...
- `main`: The main window of the application. Can be used in-game or on-desktop.
- `mode_1v1`: A window for the "1v1" mode.
- `callouts`: A window for displaying map callouts.
- `debug`: A window for OCR debugging purposes. Can be opened by double-clicking the icon for Smart-Features status on the main window.
...
3.  **Create the AppConfig.ts file**
    - Create a file named "src/AppConfig.ts"
    - Add the following code to it:
    ```typescript
    export const APP_CONFIG = {
      BACKEND_URL: 'https://fogcompanion.com/api/rpc'
    } as const;
    ```
    - This is intentionally kept out of git to test against new backend-versions on a dev-instance.
...
5.  **Load the extension in Overwolf:**
